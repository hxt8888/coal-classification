...
from dataload import MyDataset2,MyDataset2_1_test
import csv
...
def test_model(test_iter, model, device,name):
    ...
    for i, batch in enumerate(test_iter):
        feature1,feature2, target,path1,path2 = batch
        target = target[:, 0]
        with torch.no_grad():
            ...
            shibie = torch.argmax(out, dim=1) == target
            if not shibie:
                print('%s %s识别错误类别%d，正确类别%d' % (path1,path2, torch.argmax(out, dim=1).item(), target.item()))
...
if __name__ == '__main__':
    ...
    # 加载数据
    testdataset = MyDataset2_1_test('./split/Coal_test.txt',test_tfm)
    test_iter = DataLoader(dataset=testdataset, batch_size=1, shuffle=False, num_workers=20)
    '''后补'''
    traindataset = MyDataset2('./split/Coal_train.txt', train_tfm)
    train_iter = DataLoader(dataset=traindataset, batch_size=1, shuffle=True, num_workers=20)
    # 训练模型
