...
from dataload import MyDataset,MyDataset1_test
import csv
...
def test_model(test_iter, model, device,name):
    ...
    for i, batch in enumerate(test_iter):
        ...
        with torch.no_grad():
            ...
            if not shibie:
                print('%s 识别错误类别%d，正确类别%d'%(path,torch.argmax(out, dim=1).item(),target.item()))
...
if __name__ == '__main__':
    ...
    # 加载数据
    testdataset = MyDataset1_test('./split/test.txt',test_tfm,False)
    test_iter = DataLoader(dataset=testdataset, batch_size=1, shuffle=False, num_workers=0)
